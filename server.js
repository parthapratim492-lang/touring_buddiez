...
const LEGACY_PACKAGE_REDIRECTS = {
  ...
  // Kaho is now folded into the combined Dong/Tilam/Kaho circuit package —
  // send any old bookmarks/links straight to that page instead of a 404.
  '/package-kaho.html': 'dong'
};
...
app.put('/api/settings', requireAuth, (req, res) => {
  try {
    const allowed = ['phone', 'phone_raw', 'whatsapp', 'email', 'instagram', 'facebook',
                     'base_location', 'site_description', 'response_time'];
    allowed.forEach(key => {
